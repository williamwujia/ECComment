...
- 通过独立的综合 Chrome 助手批量采集淘宝、天猫和京东最新评价。
- 继续保留京东专用 Chrome 采集器，作为京东页面采集的独立兜底入口。
- 导入包含多个淘宝、天猫或京东商品的 CSV，并按 `platform + product_id` 自动拆分。
...
推荐优先安装 `ecommerce_review_assistant` 中的“电商评论更新助手”。它与原有
京东专用采集器互相独立，可以同时安装。综合助手支持混合输入三平台商品 URL，
在日常 Chrome 登录会话中逐商品采集，并输出统一 CSV。
...
遇到淘宝或天猫登录页时，队列会暂停并保留已采集检查点。扫码登录后会自动重试，
也可以在扩展中点击“登录后继续”。建议普通商品每 7 天采集一次；高活跃商品每
3–7 天，低活跃商品最长不超过 14 天。

若天猫或淘宝直达页没有加载评论模块，请先从站内正常打开一个带“查看全部评价”
的完整商品页，再点击扩展中的“采集当前商品页”。这会复用当前标签页，避免把
简化壳页或商品页的 2 条预览评价误当成完整采集结果；对直接铺在页面中的完整
评论列表，只有当前页模式会在确认识别到评论卡片后执行受控页面滚动。

完成后上传 `ecommerce_reviews_*.csv` 到“维护后台 → 数据更新”，先检查预览，
确认商品 ID、预计新增量和重复量无误后再写入。详细安装和运行说明见
[`ecommerce_review_assistant/README.md`](ecommerce_review_assistant/README.md)。

## 京东专用评论采集（兜底）

`chrome_extension` 中的原有京东采集器会继续保留，不会被综合助手替换。它使用
日常 Chrome 的现有登录会话，不读取密码、验证码或支付信息。
