...
3. 切换到“最新”；
4. 滚动评论弹层而不是商品主页面；
5. 采集昵称、星级、时间、商品名、SKU、评论正文和评论图片；
6. 达到目标后继续下一个商品；
7. 到底后连续多次没有新评论时，记录实际数量和未达标状态并继续队列。

队列和采集结果保存在 `chrome.storage.local`，扩展后台被 Chrome 暂停后仍可恢复。
全部完成后会生成两个结果，并逐商品报告目标数、实际数和短缺情况：

1. `jd_reviews_<时间>.csv`：字段与原有版本一致，直接提交给 ECComment；
2. `jd_review_images_<时间>.zip`：包含 `images.csv` 图片目录、`images/` 实际图片
   和 `README.txt`。`images.csv` 的 `review_index` 对应文本 CSV 中评论数据行的顺序，
   并记录图片原始 URL、成功使用的 URL、包内路径和下载失败原因。

图片优先尝试京东原尺寸 CDN 地址，失败时回退到页面缩略图地址。个别已失效或不在
已授权京东图片域名内的资源不会阻断整批导出，会在图片目录中标为 `failed`。
...
评论图片信息不会加入这个导入 CSV，因此现有 ECComment 导入与去重口径保持不变。
